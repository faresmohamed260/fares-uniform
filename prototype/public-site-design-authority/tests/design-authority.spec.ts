...
test("D-062 finishes textile tonal calibration",async({page})=>{
  await page.setViewportSize({width:1024,height:1536});await page.goto("/en");await page.evaluate(()=>document.fonts.ready);
  const candidates=[{name:"current",filter:"brightness(.90) saturate(.90) contrast(.95)"},{name:"b94",filter:"brightness(.94) saturate(.90) contrast(.95)"},{name:"b98",filter:"brightness(.98) saturate(.90) contrast(.95)"},{name:"b102",filter:"brightness(1.02) saturate(.90) contrast(.95)"},{name:"soft",filter:"brightness(.96) saturate(.82) contrast(.90)"},{name:"muted",filter:"brightness(.96) saturate(.75) contrast(.95)"},{name:"crisp",filter:"brightness(.94) saturate(.90) contrast(1)"}];const results:{name:string;meanAbsChannel:number;pctPixelsOver48:number}[]=[];
  for(const candidate of candidates){await page.locator(".approved-fabric-field").evaluate((node,value)=>{(node as HTMLElement).style.filter=value;},candidate.filter);const shot=(await page.screenshot({fullPage:true})).toString("base64");const metric=await page.evaluate(async({shot})=>{const load=(url:string)=>new Promise<HTMLImageElement>((resolve,reject)=>{const image=new Image();image.onload=()=>resolve(image);image.onerror=reject;image.src=url;});const [actual,reference]=await Promise.all([load(`data:image/png;base64,${shot}`),load("/authority/approved-homepage-reference.webp")]);const width=512,height=768,canvas=document.createElement("canvas");canvas.width=width;canvas.height=height;const ctx=canvas.getContext("2d",{willReadFrequently:true})!;ctx.drawImage(actual,0,0,width,height);const ap=ctx.getImageData(0,0,width,height).data;ctx.clearRect(0,0,width,height);ctx.drawImage(reference,0,0,width,height);const rp=ctx.getImageData(0,0,width,height).data;let abs=0,over48=0,pixels=0;for(let i=0;i<ap.length;i+=4){const d=(Math.abs(ap[i]-rp[i])+Math.abs(ap[i+1]-rp[i+1])+Math.abs(ap[i+2]-rp[i+2]))/3;abs+=d;pixels++;if(d>48)over48++;}return{meanAbsChannel:abs/pixels,pctPixelsOver48:over48/pixels};},{shot});results.push({name:candidate.name,...metric});}results.sort((l,r)=>l.meanAbsChannel-r.meanAbsChannel);console.log("D062_TEXTILE_FINAL_SWEEP",JSON.stringify(results));writeFileSync("artifacts/d062-textile-final-sweep.json",JSON.stringify(results,null,2));expect(results).toHaveLength(candidates.length);
});
