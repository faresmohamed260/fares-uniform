...
The current rendered implementation authority is `5826645eb17fb87cfe2399f34e439bf3bf52b74c` on `phase-9/public-site-kinetic-prototype`. Exact-head run `35461716041`, job `105946646432`, passed locked install, typecheck, optimized build and **8/8 Playwright journeys**. Review artifact `10590260486` has digest `sha256:1b727fbc5dcd2daa07f192e110b5b33882c3cdf1b04c66bd84f261ae92a7b343`. The approved boards remain documentation authority and are not runtime assets. Later Phase 9 documentation commits do not become new rendered-design authority merely by moving branch HEAD.
...
Continuation order: read this section, the Phase 9 contract, Phase 9 validation, `prototype/public-site-motion/design-qa.md` and `docs/ui/KGC_MEDIA_MANIFEST.md`; verify branch HEAD and the latest hosted run; then (1) choose the smallest professional **review-only** way to make the exact original KGC assets required by the approved boards available to the isolated hosted prototype, without inventing a production media platform; (2) replace synthetic KGC hero/stage/garment/inspector media with those originals; (3) use the D-048 annotated flat/front-back fallback whenever the originals do not contain truthful exploded layers; (4) keep Harbor House synthetic; (5) rerun hosted rendering and continue board comparison until the visual target is reached. Keep the continuity docs synchronized at each verified milestone. PR #7 stays draft. No production `apps/public-web` file, provider, storage architecture or deployment is authorized.
