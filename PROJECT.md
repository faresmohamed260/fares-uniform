...
The current rendered implementation authority remains `5826645eb17fb87cfe2399f34e439bf3bf52b74c` on `phase-9/public-site-kinetic-prototype`. Run `35461716041`, job `105946646432`, passed locked install, typecheck, optimized build and **8/8 Playwright journeys**; review artifact `10590260486` has digest `sha256:1b727fbc5dcd2daa07f192e110b5b33882c3cdf1b04c66bd84f261ae92a7b343`. It predates D-053 and therefore remains historical visual authority only.

D-053 implementation source is now `788d73c1f2341113c68ab3c8ef036c9e5589c3eb` (implementation commit `b97126216d0792739ee30f280167eb896a12c4b8` plus CI-order correction). Exact branch-head run `35517923107`, job `106096886755`, passed checkout, locked install, TypeScript typecheck and optimized production build. It then failed closed at **Stage private KGC review media** because `PHASE9_KGC_REVIEW_GOOGLE_CREDENTIALS` is not configured; Chromium and rendered Playwright journeys were skipped. No synthetic KGC fallback ran and no private Drive file was exposed. The approved boards remain documentation authority and are not runtime assets.

The accepted Astra/Pattern in Motion boards and the active comparison ledger are recorded in `prototype/public-site-motion/design-qa.md`. Fares explicitly reopened the compare-and-correct loop on 2026-09-19 and instructed iteration to continue until implementation matches those preserved boards. The next correction is now explicit: KGC client data must use the original KGC media catalogued in `docs/ui/KGC_MEDIA_MANIFEST.md`; synthetic KGC people, garments or fabricated construction layers are not acceptable as KGC client data. Harbor House remains the clearly synthetic generalization proof. This requirement changes the Phase 9 review-media policy only; it does **not** select or redesign production media storage, a CDN, object storage, CMS, or ingestion architecture. Google Drive remains the audited source location for the existing KGC originals, not an approved public runtime asset service. GitHub is not to be turned into a client-photo warehouse. If the isolated hosted prototype needs a review-only delivery mechanism for a small authorized asset subset, that mechanism must be chosen narrowly inside Phase 9 without being promoted into production architecture. Functional, responsive, RTL, keyboard and reduced-motion gates remain GREEN while visual parity continues.

Continuation order: read this section, the Phase 9 contract, Phase 9 validation, `prototype/public-site-motion/design-qa.md` and `docs/ui/KGC_MEDIA_MANIFEST.md`; verify branch HEAD and the latest hosted run. The review-only delivery mechanism is now implemented: six exact private Drive files are staged ephemerally on the hosted runner, never committed or deployed, and KGC uses original Summer worn anchors plus the High/Summer front/back packshots while Harbor House remains synthetic. The only current external blocker is the absent review credential. Configure a disposable review identity that can read only those six Drive files and store its service-account JSON as the GitHub Actions secret `PHASE9_KGC_REVIEW_GOOGLE_CREDENTIALS`; then rerun the exact hosted workflow, inspect desktop/mobile/RTL/inspector evidence against the preserved boards, and continue correcting visual mismatches. PR #7 stays draft. No production `apps/public-web` file, provider, storage architecture or deployment is authorized.
...
Current rendered visual-parity authority is `5826645eb17fb87cfe2399f34e439bf3bf52b74c`. Hosted run `35461716041`, job `105946646432`, passed locked install, typecheck, optimized build and **8/8 Playwright journeys**; review artifact `10590260486` has digest `sha256:1b727fbc5dcd2daa07f192e110b5b33882c3cdf1b04c66bd84f261ae92a7b343`.

The approved-design compare-and-correct loop is active by Fares's explicit instruction. The four pre-implementation boards are immutable comparison inputs in every review artifact. Functional/accessibility QA is GREEN, but visual parity remains open. D-053 now requires the KGC review fixture to use the original manifest-backed KGC visual set rather than synthetic KGC people/garments or fabricated construction layers. Harbor House remains the clearly synthetic non-school proof. This is a Phase 9 fidelity/content correction only; it does not choose a production media-storage/CDN/CMS architecture.

The principal remaining Phase 9 work is the original-KGC-media fidelity checkpoint followed by continued preserved-board comparison. PR #7 remains draft. Do not merge it, deploy the prototype, edit production `apps/public-web`, invent a production media-storage architecture, or begin production implementation without the corresponding separate authorization.
