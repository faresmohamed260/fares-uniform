...
## D-043 — Transaction-lock quarantine for the pre-existing staging cron backlog

Status: Accepted bounded staging-proof policy, 2026-09-16; no production or ordinary-cron execution authorization.

The isolated staging database contains 19 active ordinary Odoo/Fares cron jobs already due with `lastcall = NULL`. Several can have accounting, stock, messaging or external-service side effects. They must not be run, disabled, postponed, rescheduled or drained merely to permit a synthetic locking proof.

For the Phase 8 external-cron acceptance proof only, a separate bounded transaction may acquire `FOR NO KEY UPDATE` locks on every pre-existing active ordinary `ir_cron` row after validating the diagnosed backlog and exact database epoch. Pinned Odoo 19 acquires ready jobs with `FOR NO KEY UPDATE SKIP LOCKED`; therefore proof runtimes skip those quarantined rows and may acquire only the synthetic job created after quarantine is ready. The transaction must remain open through trigger and synthetic cleanup, then roll back. Fail closed if the backlog boundary or ordinary-cron snapshot changes, quarantine cannot be established, cleanup fails, or the branch/deployment/database identities differ.

Acceptance requires exact pre/post snapshots of all ordinary cron and ordinary trigger rows, zero business-data deltas, no synthetic residue, and the existing privilege/provider seal. This policy does not authorize routine backlog handling, ordinary cron execution, a scheduler rollout, application redeployment, database reset, paid resources or production action. GREEN authority: workflow source `59d9553a1f14b6f4a1136ae4089d5289dcefd0e1`, run `35026879234`, job `104575905196`.
## D-044 — Review-only release integration checkpoint
...
For the isolated Phase 9 KGC fixture, the manifest-backed KGC worn/model anchors, matched/used packshots, canonical crest/logo and campus exterior recorded in `docs/ui/KGC_MEDIA_MANIFEST.md` are the authorized visual source set. KGC client-facing hero, stage, garment and inspection media must resolve only to those original assets. Synthetic people, synthetic garments and fabricated KGC construction layers must not be presented as KGC client data.

This decision is a **content/fidelity authorization**, not a media-storage architecture decision. It does not direct the project to store the KGC library in GitHub, serve public/runtime assets from Google Drive, adopt Cloudflare R2, adopt Supabase Storage, add a CMS, or otherwise create a production media platform. Google Drive remains the audited source location for the existing originals. Any mechanism used only to deliver the small subset needed by the isolated hosted Phase 9 review must remain review-scoped unless a later architecture decision explicitly promotes it.
