...
The operations contract is `docs/operations/PHASE_8_STAGING_OBSERVABILITY.md`. Its owner labels are roles (`technical-operations-owner`, `business-operations-owner`) rather than invented people. PR #6 placed the twice-hourly workflow on default branch `main`, so recurring GitHub staging polling is active. Manual run `35099333740` / job `104804477793` and native scheduled run `35129331378` / job `104906119150` are GREEN; the scheduled event used exact source `c2a93d8d693f47ebc7b286af5eaebb485895d8c0`. Named production owners, recipients/escalation and retention remain Gate D.
...
**NO-GO.** The selected production direction is Vercel application hosting + Cloudflare authoritative DNS/edge + Supabase PostgreSQL, using owned domain `faresuniform.uk`. Apex/`www` DNS-only wiring, Vercel verification, TLS, the apex canonical redirect and EN/AR no-price/no-stock rendering are GREEN. Production still requires approval of the production Vercel plan/spending boundary; final Cloudflare proxy policy and DNS/certificate/private-access ownership; store-device acceptance; named staff/roles/training; separately authorized real-data cutover/reconciliation; production secrets/private access; backup retention and RPO/RTO decisions; named monitoring owners/recipients/escalation; production log/alert retention; launch authorization; and an explicit client production GO.

## Immediate continuation

Follow [PROJECT.md](../../PROJECT.md) for the ordered current tasks, [repair evidence](../validation/PHASE_8_HANDOFF_REPAIR.md) for exact Gate C runs and the [production domain plan](../operations/PRODUCTION_DOMAIN_PLAN.md) for custom-domain evidence. Reverify branch HEAD before every write. Gate C and custom-domain technical wiring need no further proof under the accepted staging boundary; do not install `fu_uat` in staging. Gate D remains separately authorized.
