...
**Mandatory public-site UI/UX authority rule:** read `docs/ui/REPO_FIRST_UI_UX_WORKFLOW.md`, the active page-specific authority, and `.agents/skills/fares-scrollcraft/SKILL.md` before public-site visual work. D-059 is the general browser-native rule, but **D-062 explicitly supersedes it for the homepage**: the committed approved homepage image and its one-to-one component map are the exact visual implementation authority until Fares changes that decision.

The project normally uses a repo-first, browser-native design process with no required paid design SaaS. **Homepage exception D-062:** Fares has explicitly approved one generated homepage board as a pixel-level visual specification. Implement that board one-to-one from the committed image/component map, then use browser screenshots and image-difference evidence to converge on fidelity. This exception does not automatically apply to other pages.
