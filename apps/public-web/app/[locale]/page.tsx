...
export default async function Home({ params, searchParams }: Props) {
  ...
  return <>
    ...
    <main id="main-content">
      ...
      <section className="process-section" id="process"><div><span className="eyebrow">{locale === "ar" ? "طريقة العمل" : "How it moves"}</span><h2>{text.process}</h2></div><ol>{text.steps.map((step, index) => <li key={step}><span>0{index + 1}</span><strong>{step}</strong></li>)}</ol><p className="trust-line">{text.trust}</p></section>
      <EnquiryForm
        language={locale}
