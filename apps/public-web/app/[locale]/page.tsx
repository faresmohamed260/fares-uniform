...
import { getV2Home } from "@/lib/public-v2";
...
export default async function Home({ params }: Props) {
  ...
  const [homeData, items] = await Promise.all([getV2Home(locale), getCatalog(locale)]);
  return <>
    ...
    <main id="main-content">
      ...
      <section className="hero"><div className="hero-copy"><span className="pill">{text.badge}</span><h1>{text.hero}</h1><p>{text.sub}</p><div className="hero-actions"><Link className="primary-button" href={`/${locale}/work`}>{text.explore}<span aria-hidden="true">↘</span></Link><a className="secondary-button" href="#enquiry">{text.talk}</a></div></div><div className="hero-art" aria-hidden="true"><div className="fabric-card fabric-a"><span>01</span></div><div className="fabric-card fabric-b"><span>FU</span></div><div className="fabric-line"/></div></section>
      <section className="work-section" aria-labelledby="selected-work-title"><span className="eyebrow">{text.selected}</span><div className="section-heading"><h2 id="selected-work-title">{text.selectedTitle}</h2><span className="section-count">0{homeData.featured_work.length}</span></div><div className="work-grid">{homeData.featured_work.map((item) => <article className="work-card" key={`${item.organization.slug}/${item.program.slug}`}><div><span className="work-meta">{item.organization.sector}</span><h3>{item.organization.name} · {item.program.title}</h3><p>{item.program.summary}</p></div><Link className="text-link" href={`/${locale}/work`}>{text.explore}<b aria-hidden="true">↗</b></Link></article>)}</div></section>
